...
\noindent%
{\it Keywords:} Model-based clustering, trimming, heterogeneous clusters %% without formatting
\spacingset{1.45}
...
concentrated outliers which are specially dangerous in (non-robust)
estimation. \cite{GarG10} provides a recent survey of robust
clustering methods.
...
and \cite{GarG08}. Trimming also serves to highlight interesting
...
clustering results are shown in Figure \ref{f1}. Among this $3\%$ of
...
#####################
## Initialization  ## # has to be run before any of the examples
#####################
...
always denoted by the symbol ``$\circ$" (in all the figures in this
work).} \label{f1}
\end{figure}
...
\pkg{mclust} \citep{BanR93,FraR98} and the Fortran program
\pkg{emmix} \citep{LacP00} implement such robust mixture fitting
...
adequate cluster scatter matrix constraints leading to a wide range
of clustering procedures, depending on the chosen constraint, and
avoiding the occurrence of spurious non-interesting clusters.
...
are given in Section \ref{sec:alg}. Some comments about how
\pkg{tclust} performs compared to other robust clustering approaches
are given in Section \ref{sec:comp}. Section \ref{sec:kal} shows
some graphical outputs that help us make sensible choices for the
...
(\ref{spu}) can be simplified to the same search (of a partition,
vectors and positive definite matrices) but just maximizing
...
that the maximization of (\ref{e4}) becomes a well-defined problem.
Note that equation (\ref{e4}) simplifies to (\ref{e3}) when assuming
\code{equal.weights = TRUE} and all weights are equally set to
$\pi_j=1/k$.
...
scattered" clusters. The idea of constraining cluster scatters to
...
This type of constraints limits the relative size of the axes of the
...
Now, this type of constraints limits the relative volumes of the
mentioned equidensity ellipsoids, but not the cluster shapes. The
use of this type of constraints is particularly advisable when
affine equivariance is required because this property is satisfied
when \code{restr = "deter"}.
...
<<fig=TRUE,echo=false,results=hide,label=chunk2a,width=7.14,height=7.8>>=
...
<<fig=TRUE,echo=false,results=hide,label=chunk2,width=8,height=9>>=
################
##  Figure 3  ##
################
...
log-likelihood objective function (\ref{e3}) or (\ref{e4})
...
<<fig=TRUE,echo=false,results=hide,label=chunk3a,width=7.14,height=7.8>>=
################
##  Figure 4  ##
################
...
\code{equal.weights = FALSE}.} \label{f3a}
\end{figure}

Sometimes equations (\ref{e3}) and (\ref{e4}) maximize with some
clusters remaining empty (see Figure \ref{f3a}). In this case, only
information on the non-empty groups is returned. Notice that, if we
are searching for $k$ clusters, empty clusters can be found when a
clustering solution for a number of clusters strictly smaller than
$k$ attains a higher value for (\ref{e3}) or (\ref{e4}) than the
best solution found with $k$ clusters. In this case, artificial
empty clusters may be defined by considering sufficiently remote
centers $\mu_j$ and scatter matrices $\Sigma_j$ satisfying the
specific constraints that are assigned to these empty clusters. They
are chosen such that $f(\cdot;\mu_j,\Sigma_j)$ gives almost null
density to all the observations in the sample. These artificially
added centers and scatter matrices are not returned as output by the
\code{tclust} function and a warning is issued. For instance, let us
...
R > plot (clus)
\end{Scode}
Although we are searching for $k=3$ clusters, Figure \ref{f3a} and
...
will see in Section \ref{sec:kal}. On the other hand, the detection
...
(\ref{e3}) or (\ref{e4}) increases with larger values of
...
constraints \citep{GarG08}. A more detailed description of the
algorithm applied by \code{tclust} can be found in \cite{FriG11}.

\section{Comparison with other robust clustering
proposals}\label{sec:comp}
...
left corner of Figure \ref{f1} do not affect the position of the
$k$-medoid centers (see Figure \ref{fig:pam},(a)) too much, nor the
resulting clusters. However, in Figure \ref{fig:pam},(b), we see
...
the other hand, in Figure \ref{fig:pam},(c), we can see that these
outlying data points do not affect the trimmed $k$-means based
clustering at all once that they are trimmed.

\begin{figure}[t!]
\centering
%\includegraphics[width=16cm]{plots_R-chunk_ex_pam} %1,3
<<fig=TRUE,echo=false,results=hide,label=chunk_ex_pam,width=9,height=5>>=
################
##  Figure 5  ##
################
...
in Figure \ref{fig:elt}. These two data sets contain three main
...
non-normally distributed components as those in Figure \ref{fig:elt}
may result in proper clustering solutions, this however cannot be
guaranteed if the underlying distributions differ too much from
normality. The closely related \pkg{tlemix} package allows to
consider other non-normal models by taking advantage of the
flexibility provided by the \pkg{FlexMix} package \citep{LeiF04}. On
the other hand, \pkg{tclust} focuses on normally distributed
components and on the implementation of appropriate cluster scatter
matrix constraints while \pkg{tlemix} does not. The \pkg{tlemix}
mainly controls the minimum number of observations in a cluster.

\begin{figure}[t!]
\centering
%\includegraphics[width=14cm]{plots_R-chunk_ex_elt} %2,3
<<fig=TRUE,echo=false,results=hide,label=chunk_ex_elt,width=9,height=6.5>>=
################
##  Figure 6  ##
################
...
$k=3$ and $\alpha=0.05$ in (c) and (f).} \label{fig:elt}
\end{figure}
...
``background noise" as seen in Figure \ref{fig:mclust},(a). However,
it is not able to cope with a more structured noise pattern like the
``helix" in Figure \ref{fig:mclust},(c) which also accounts for
$10\%$ of the data, although the information of a $10\%$
contamination level was passed to \pkg{mclust}. Alternatively, the
\pkg{tclust} package with $k = 2$ and $\alpha = 0.1$ properly
discovers the outlying data points without trying to fit them.

\begin{figure}[t!]
\centering
%\includegraphics[width=11cm]{plots_R-chunk_ex_mclust}
<<fig=TRUE,echo=false,results=hide,label=chunk_ex_mclust,width=7.14,height=7.8>>=
################
##  Figure 7  ##
################
...
with $k=2$ and $\alpha=0.1$ in (b) and (d).} \label{fig:mclust}
\end{figure}
...
data set in Figure \ref{f3} which could either be interpreted as a
...
Figure \ref{f3} are perfectly sensible and the final choice of
$\alpha$ and $k$ only depends on the value given to
\code{restr.fact}. The code used to obtain Figure \ref{f3} is the
following:
\begin{Scode}
...
<<fig=TRUE,echo=false,results=hide,label=chunk3,width=10,height=5.45>>=
################
##  Figure 8  ##
################
...
$\alpha=0.1$ and \code{restr.fact} $=8$ (b).} \label{f3}
\end{figure}
...
first $\alpha_0$ such that (\ref{diff}) is (close to) 0 for every
...
determining when (\ref{diff}) can be formally considered as ``close
to 0".
...
\end{Scode}
results in Figure \ref{f4}. This figure shows that increasing $k$
from 2 to 3 is needed when $\alpha=0$, as the objective functions
value differs noticeably between $k = 2$ and $k = 3$. On the other
hand, increasing $k$ from 2 to 3 is not needed anymore as the third
(more scattered) ``cluster" vanishes when trimming 5\% of the most
...
Increasing $k$ from 3 to 4 is not needed in any case.

\begin{figure}[t!]
\centering
%\includegraphics[width=10cm]{Plots_R-chunk4} %   1,1
<<fig=TRUE,echo=false,results=hide,label=chunk4,width=7.14,height=6.25>>=
################
##  Figure 9  ##
################
...
value \code{restr.fact = 50} has been chosen for \code{ctlcurves},
as this is surely enough for most of the applications (specially if
the variables have been properly standardized through the
\code{scale} argument in the \code{tclust} function). This way, we
would obtain some few sensible values for $k$ and $\alpha$ such that
their associated clustering solutions must be carefully explored.
For instance, \code{tclust} issues a warning when the returned
clustering solution has been ``artificially restricted" by the
algorithm, which means that the restricted and unrestricted cluster
scatter matrices after the last concentration step do not
coincidence (see an example of this in Section \ref{sec:exa}). In
this situation, if no specific constraints are required,
\code{restr.fact} may be increased stepwise until this warning
disappears. In this way, we would be searching for a clustering
solution which is not artificially restricted and does not contain
spurious clusters. In any case, the exploratory tools in Section
\ref{sec:gra} also help to evaluate whether all these parameters are
reasonably chosen.
...
implemented in the package \pkg{fpc} \citep{Hen10}.
\end{description}
...
\end{Scode}
yields Figure \ref{f5a}. For demonstrating the different plotting
...
a three-dimensional data set (\code{geyser3}). Figure \ref{f5a}
...
<<fig=TRUE,echo=false,results=hide,label=chunk5a,width=10,height=5.45>>=
#################
##  Figure 10  ##
#################
...
obtained by maximizing  (\ref{e3}) or (\ref{e4}) (we set
$\widehat{\pi_j}=1/k$ when maximizing (\ref{e3})).
...
with cluster $j$ to the best second possible assignment. That is
...
for }x_i\text{ trimmed}.
$$
Hence, discriminant factors $\text{DF}(i)$ are
obtained for every observation in the data set, whether trimmed or
not.

\begin{figure}[t!]
\centering
%\includegraphics[width=14cm]{Plots_R-chunk5}
<<fig=TRUE,echo=false,results=hide,label=chunk5,width=9,height=6.5>>=
#################
##  Figure 11  ##
#################
...
plot.DiscrFact.p3 (discr.clus.w, tol.lty = 2, main.pre = "(c)")
...
Figure \ref{f5} shows the result of applying the \code{DiscrFact}
function to a clustering solution found for the \code{mixt} data set
appearing in Figure \ref{f3}. The following code is used to obtain
this figure:
...
$D_{(k)}(x_{(\rounding{n\alpha}+1)};\widehat{\theta},\widehat{\pi})$
for trimmed observations) is smaller than one tenth of the quality
of the actually made decision
($D_{(k)}(x_i;\widehat{\theta},\widehat{\pi})$).

Although Figure \ref{f4} suggests to choose $k = 2$, $k$ has been
...
\code{DiscrFact}. Figure \ref{f5},(a) simply illustrates the cluster
...
<<fig=TRUE,echo=false,results=hide,label=chunk6,width=7.14,height=6.25>>=
#################
##  Figure 12  ##
#################
...
<<fig=TRUE,echo=false,results=hide,label=chunk7,width=9,height=6.5>>=
#################
##  Figure 13  ##
#################
...
restriction factor (40) does.                                      %Notice also that variables in this data set are not standardized and, thus, a large value of \code{restr.fact} is needed. <- This sentence should be removed because the reviewer could ask us why the data set was not standardized...
By examining the sizes of the obtained groups, we see that no
spurious groups are found with \code{restr.fact = 50}:
\begin{Scode}
R > clus$size
  [1] 95 85
\end{Scode}
...
<<fig=TRUE,echo=false,results=hide,label=chunk8,width=9,height=6.5>>=
#################
##  Figure 14  ##
#################
